...
FROM (
    SELECT 
        ...
        RANK() OVER (
            PARTITION BY EXTRACT(YEAR FROM v.fechaDespegue) 
            ORDER BY COUNT(*) DESC
        ) AS rnk
    FROM Cliente c
    ...
    WHERE c.nacionalidad = 'Chileno'
      AND v.fechaDespegue >= DATE_TRUNC('year', NOW()) - INTERVAL '3 years'
    GROUP BY EXTRACT(YEAR FROM v.fechaDespegue), v.destino
) ranked
WHERE rnk = 1;
...
WHERE v.fechaDespegue >= DATE_TRUNC('year', NOW()) - INTERVAL '3 years'
GROUP BY mes, pais
...
WHERE 
    ...
    AND s.fechaPago >= DATE_TRUNC('year', NOW()) - INTERVAL '3 years'
GROUP BY 
    anio, e.nombre
ORDER BY 
...
WHERE sueldo.fechapago >= DATE_TRUNC('year', NOW()) - INTERVAL '9 years'
